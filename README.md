...
#### 1. <span title='CMD, PowerShell, Linux용 터미널 등'>터미널</span>에서 ```2025-2-CFD``` 폴더에 접근합니다. 

'cd' 뒤에 원하는 폴더의 상대경로 또는 절대경로를 붙여 ```cd user``` 등의 형태로 명령어를 사용하여 원하는 폴더에 접근할 수 있습니다.

또는 파일 탐색기에서 ```2025-2-CFD``` 폴더에서 우클릭 후 '터미널에서 열기'를 마우스로 선택합니다.
...
Initialize를 제외한 파일 관리 항목들은 ```check```에서 지원하지 않습니다. 단일 케이스 디렉토리이기 때문에 별도의 파일 관리 스크립트가 필요하지 않습니다.
...
마찬가지로 터미널을 이용하여 ```main.py```에서 원하는 폴더에 접근하여 원하는 작업의 번호를 입력합니다.
